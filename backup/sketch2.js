...
const sketch = (p) => {
  ...
  function addSection() {
    ...
    const layout = new Layout(artist.artist, copyParam.copy);
    layout.addPage();
    layout.addCopy();
  ...
  class Layout {
    constructor(artist, copy, seed = 100, numCols = 8, numRows = 8, margin = 20) {
      this.artist = artist;
      this.copy = copy;
      this.seed = seed;
      this.margin = margin;
    ...
    setGrid() {
      ...
      this.categories = ["image", "copy"];
      for (let i = 0; i < this.categories.length; i++) {
        let startPos = this.getRandomEmptyCell();
        this.walkers.push({
          x: startPos.x,
          y: startPos.y,
    ...
    addCopy() {
      ...
      while (currentBox) {
        ...
        while (currentBox.elt.scrollHeight > currentBox.elt.clientHeight) {
          ...
          const tokens = content.split(/(\s+)/); // Split on whitespace, include it in tokens
          let overflowTokens = [];
